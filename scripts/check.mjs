...
assert(subscribe.includes(`href="${feedUrl}"`), 'Subscription page must offer the journal feed');
const home = readFileSync(join(output, 'index.html'), 'utf8');
const subscribeUrl = `${prefix}subscribe/`.replace('//', '/');
assert(home.includes('class="intro-actions"') && home.includes(`href="${subscribeUrl}"`), 'Homepage hero must feature the email updates action');
